...
// Frontend type — uses camelCase as normal for React
interface PastedImage {
  id: string
  url: string      // was `dataUrl` — now matches backend ImageData.url directly
  width: number
  height: number
}

// Backend ImageData shape (matches Rust struct exactly)
...
// No conversion needed — frontend PastedImage.url === backend ImageData.url
function toBackendImage(img: PastedImage): BackendImage {
  return { id: img.id, url: img.url, width: img.width, height: img.height }
}

function fromBackendImage(img: BackendImage): PastedImage {
  return { id: img.id, url: img.url, width: img.width, height: img.height }
}
    ...
    try {
      const res = await fetch('/api/snippets', {
        ...
        body: JSON.stringify({
          ...
          content: code,
          language,
          // toBackendImage is now a no-op shape-wise, but kept for explicitness
        ...
        <div className="px-5 py-5 flex flex-col gap-4">
          ...
          {sharedUrl ? (
            <div className="flex flex-col gap-3">
              ...
              <button
                onClick={onClose}
                className={`text-sm py-2 rounded border ${isDark ? 'border-[#555] text-gray-300 hover:bg-[#2d2d2d]' : 'border-gray-300 text-gray-600 hover:bg-gray-50'}`}
              >Close</button>
            </div>
          ) : (
            <button
...
export default function App() {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (msg.type === 'connected') {
            ...
            // Backend sends { id, url, width, height } — matches PastedImage directly
            const img = fromBackendImage(msg.image as BackendImage)
    ...
    fetch(`/api/snippets/${encodeURIComponent(slug)}`)
      ...
      .then(d => {
        ...
        if (Array.isArray(d.images) && d.images.length > 0) {
          // Backend returns { id, url, width, height } — no conversion needed
  ...
  // ── Process image file (paste or drop) ────────────────────────────────────
    ...
    reader.onload = ev => {
      ...
      image.onload = () => {
        const newImg: PastedImage = {
          ...
          url,                          // base64 dataUrl stored as `url`
          width: image.naturalWidth,
          height: image.naturalHeight,
        }
        setPastedImages(prev => [...prev, newImg])
        // Send to backend — { type: "image", image: { id, url, width, height } }
    ...
    >
      ...
      {loadError && (
        <div className="bg-red-900/40 border-b border-red-800 text-red-300 text-xs px-5 py-2 flex items-center justify-between">
          <span>{loadError}</span>
          <a href="/" className="underline ml-2">Start new</a>
        </div>
      )}
